...
def gitDescribeToPep440(version):
    ...
    VERSION_MATCH = re.compile(r'(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)(\.post(?P<post>\d+))?(\.flathub(?P<flathub>\d+))?(-(?P<dev>\d+))?(-g(?P<commit>.+))?') # noqa pylint: disable=line-too-long
    v = VERSION_MATCH.search(version)
    if v:
        major = int(v.group('major'))
        minor = int(v.group('minor'))
        patch = int(v.group('patch'))
        res = "{}.{}.{}".format(major, minor, patch)
        if v.group('dev'):
            patch += 1
            dev = int(v.group('dev'))
            res = "{}-dev{}".format(res, dev)
        elif v.group('post'):
            res = "{}.post{}".format(res, v.group('post'))
        if v.group('flathub'):
            res = "{}+flathub{}".format(res, v.group('flathub'))
        return res
